...
f=open('/media/Scratch_SSD_Voyager/sammi/RNA_editing/fc_OD_v1/counts_all_tpm.txt')
tpm_list={}
for i in f: 
  if i.startswith('Geneid'):
    h=i.strip().split('\t')
    for j in range(len(h)):
      if h[j].startswith('X.media.Scratch_SSD_Voyager.sammi.RNA_editing.OrganDev_v1.nomultiseg'):
        sn_in = h[j].strip().split("_")
        for k in range(len(sn_in)):
          if sn_in[k].startswith('v1'): sn_p1 = sn_in[k]
#          if sn_in[k].startswith('ter'): sn_p2 = sn_in[k]
        sn = sn_p1.replace('.', '_')
        tpm_list[sn] = []
        if h[j].startswith('X.media.Scratch_SSD_Voyager.sammi.RNA_editing.OrganDev_'+sn_p1): tpm_list[sn].append(j)
    continue
  l=i.strip().split('\t')
