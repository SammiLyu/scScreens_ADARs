...
##
lm.pert<-function(dat,form,perturbations="perturbations",batch="batch",numRep=1000,useBatch=F,dep_var="DPT",ref="",minPert=10,verbose=T)
{
  ...
  if(nchar(ref)>1)
  {
    ...
    dat <- within(dat, perturbations <- relevel(perturbations, ref = ref))
  }
